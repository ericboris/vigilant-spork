...
if __name__ == '__main__':
	...
	with open(join('/content/drive/MyDrive/CSE447_Project_cp3/', train_file, 'r') as f:
		text = f.read()
	...
	if os.path.isfile(join('content/drive/MyDrive/CSE447_Project_cp3/', f'RNN-{n_hidden}-{n_layers}-{batch_size}-{seq_len}-{lr}-{clip}.net')):
	...
	# Else train new model.	
		...
		print('Saving model')
		with open(join('/content/drive/MyDrive/CSE447_Project_cp3/', f'RNN-{n_hidden}-{n_layers}-{batch_size}-{seq_len}-{lr}-{clip}.net'), 'wb') as f:
	...
	with open(join('/content/drive/MyDrive/CSE447_Project_cp3/', f'RNN-{n_hidden}-{n_layers}-{batch_size}-{seq_len}-{lr}-{clip}.net'), 'rb') as f:
		checkpoint = torch.load(f)

	loaded = CharRNN(checkpoint['tokens'], n_hidden=checkpoint['n_hidden'], n_layers=checkpoint['n_layers'])
	loaded.load_state_dict(checkpoint['state_dict'])

	print(f'Loading test dat')	
	test_data = load_test_data('/content/drive/MyDrive/CSE447_Project_cp3/input.txt')
	...
	model_name = f'RNN-{n_hidden}-{n_layers}-{batch_size}-{seq_len}-{lr}-{clip}-{train_file}'
	write_pred(results, f'/content/drive/MyDrive/CSE447_Project_cp3/{model_name}')
